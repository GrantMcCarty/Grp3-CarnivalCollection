...
public class Rifle:MonoBehaviour
{
    ...
    void FireProjectile() {
        ...
        RaycastHit hit; 
        if (Physics.Raycast(barrelEnd.transform.position, transform.TransformDirection(Vector3.up), out hit, Mathf.Infinity))
        {
            GameObject thing = hit.collider.gameObject;
            if(debug) {
                GameObject player = GameObject.FindWithTag("Player");
                player.GetComponent<FPSController>().canMove = false;
                Ray ray = new Ray(barrelEnd.transform.position, transform.TransformDirection(Vector3.up));  
